...
class TechnoGenrePage extends ConsumerStatefulWidget {
  const TechnoGenrePage({super.key});

  @override
  ConsumerState<TechnoGenrePage> createState() => _TechnoGenrePageState();
}

class _TechnoGenrePageState extends ConsumerState<TechnoGenrePage>
    with TickerProviderStateMixin {
  static const _pageBackground = Color(0xFF111111);
  static const _panelBackground = Color(0xFF1A1A1A);
  static const _technoQuery = 'Techno';
  static const _technoGenreQueries = [
    'Techno',
    'techno',
  ];
  ...
  Future<void> _fetchTrendingTracks() async {
    try {
      final resp = await dioClient.dio.get(
        '/discovery/trending',
        queryParameters: {'genre': _technoQuery},
      );
  ...
  Future<void> _fetchPlaylists() async {
    try {
      debugPrint('[TechnoGenrePage] _fetchPlaylists: requesting genre="Techno"');
      final resp = await dioClient.dio.get(
        '/playlists',
        queryParameters: {'genre': _technoQuery, 'limit': 10},
      );
      debugPrint('[TechnoGenrePage] _fetchPlaylists raw: ${resp.data}');
      if (!mounted) return;
      setState(() {
        ...
        children: [
          ...
          if (previewTracks.isEmpty && recentTracks.isEmpty && _playlists.isEmpty)
            const Padding(
              ...
              child: Text(
                'No Techno content available right now.',
      ...
      children: [
        ...
        if (_isPlaylistsLoading)
          ...
          const Padding(
            ...
            child: Text(
              'No Techno playlists available right now.',
        ...
        if (_isAlbumsLoading)
          ...
          Padding(
            ...
            child: _buildInlineRetryCard(
              message: 'Failed to load Techno albums.',
          ...
          const Padding(
            padding: EdgeInsets.symmetric(horizontal: 16, vertical: 8),
            child: Text(
              'No Techno albums available right now.',
              style: TextStyle(color: Colors.white54, fontSize: 14),
            ),
          )
        else
      ...
      children: [
        ...
        if (_isLoading)
          ...
          Padding(
            padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
            child: _buildInlineRetryCard(
              message: 'Failed to load Techno artist profiles.',
          ...
          const Padding(
            padding: EdgeInsets.symmetric(horizontal: 16, vertical: 8),
            child: Text(
              'No Techno artist profiles available right now.',
              style: TextStyle(color: Colors.white54, fontSize: 14),
            ),
          )
        else
      ...
      children: [
        ...
        if (_isLoading)
          ...
          Padding(
            padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
            child: _buildInlineRetryCard(
              message: 'Failed to load more Techno tracks.',
              onRetry: _fetchGenreTracks,
            ),
          )
          ...
          const Padding(
            padding: EdgeInsets.symmetric(horizontal: 16, vertical: 8),
            child: Text(
              'No more Techno tracks available right now.',
              style: TextStyle(color: Colors.white54, fontSize: 14),
            ),
          )
        else
  ...
  Widget _buildInlineRetryCard({
    ...
    String message = 'Failed to load Techno playlists.',
  }) {
    return Container(
      width: double.infinity,
