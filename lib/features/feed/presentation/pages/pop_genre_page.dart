...
class PopGenrePage extends ConsumerStatefulWidget {
  const PopGenrePage({super.key});

  @override
  ConsumerState<PopGenrePage> createState() => _PopGenrePageState();
}

class _PopGenrePageState extends ConsumerState<PopGenrePage>
  ...
  Future<void> _fetchTrendingTracks() async {
    try {
      final resp = await dioClient.dio.get(
        '/discovery/trending',
        queryParameters: {'genre': _popQuery},
      );
  ...
  Future<void> _fetchPlaylists() async {
    try {
      debugPrint('[PopGenrePage] _fetchPlaylists: requesting genre="Pop"');
      final resp = await dioClient.dio.get(
        '/playlists',
        queryParameters: {'genre': _popQuery, 'limit': 10},
      );
      debugPrint('[PopGenrePage] _fetchPlaylists raw: ${resp.data}');
      if (!mounted) return;
      setState(() {
        ...
        children: [
          ...
          if (previewTracks.isEmpty && recentTracks.isEmpty && _playlists.isEmpty)
            const Padding(
              ...
              child: Text(
                'No Pop content available right now.',
  ...
  void _openIntroducingPlaylist() {
    if (_playlists.isEmpty) {
      _pushIntroducingPlaylist('/search/pop/introducing');
      return;
    }
    ...
    final selectedPlaylist = _playlists.firstWhere(
      (playlist) {
        ...
        return title.contains('buzzing pop') ||
            title.contains('pop buzzing') ||
    ...
    _pushIntroducingPlaylist('/search/pop/introducing?playlistId=$playlistId');
  }
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  gradient: RadialGradient(
                    ...
                    radius: 1.22,
                    colors: [
                      const Color(0xFFFFF2A8).withValues(alpha: 0.96),
                      const Color(0xFFFFDE59).withValues(alpha: 0.88),
                      const Color(0xFFE5B72F).withValues(alpha: 0.68),
                      const Color(0xFF8C6A18).withValues(alpha: 0.46),
                      const Color(0xFF3E3213).withValues(alpha: 0.24),
                    ],
  ...
  Widget _buildPlaylistListCard(_PlaylistInfo playlist) {
    ...
    return GestureDetector(
      onTap: () => context.push(
                '/search/pop/introducing?playlistId=${Uri.encodeComponent(playlist.id)}',
              ),
      child: Container(
      ...
      children: [
        ...
        if (_isPlaylistsLoading)
          ...
          const Padding(
            ...
            child: Text(
              'No Pop playlists available right now.',
        ...
        if (_isAlbumsLoading)
          ...
          Padding(
            ...
            child: _buildInlineRetryCard(
              message: 'Failed to load Pop albums.',
          ...
          const Padding(
            padding: EdgeInsets.symmetric(horizontal: 16, vertical: 8),
            child: Text(
              'No Pop albums available right now.',
              style: TextStyle(color: Colors.white54, fontSize: 14),
            ),
          )
        else
      ...
      children: [
        ...
        if (_isLoading)
          ...
          Padding(
            padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
            child: _buildInlineRetryCard(
              message: 'Failed to load Pop artist profiles.',
          ...
          const Padding(
            padding: EdgeInsets.symmetric(horizontal: 16, vertical: 8),
            child: Text(
              'No Pop artist profiles available right now.',
              style: TextStyle(color: Colors.white54, fontSize: 14),
            ),
          )
        else
      ...
      children: [
        ...
        if (_isLoading)
          ...
          Padding(
            padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
            child: _buildInlineRetryCard(
              message: 'Failed to load more Pop tracks.',
              onRetry: _fetchGenreTracks,
            ),
          )
          ...
          const Padding(
            padding: EdgeInsets.symmetric(horizontal: 16, vertical: 8),
            child: Text(
              'No more Pop tracks available right now.',
              style: TextStyle(color: Colors.white54, fontSize: 14),
            ),
          )
        else
  ...
  Widget _buildInlineRetryCard({
    ...
    String message = 'Failed to load Pop playlists.',
  }) {
    return Container(
      width: double.infinity,
