...
class HouseGenrePage extends ConsumerStatefulWidget {
  const HouseGenrePage({super.key});

  @override
  ConsumerState<HouseGenrePage> createState() => _HouseGenrePageState();
}

class _HouseGenrePageState extends ConsumerState<HouseGenrePage>
    with TickerProviderStateMixin {
  static const _pageBackground = Color(0xFF111111);
  static const _panelBackground = Color(0xFF1A1A1A);
  static const _houseQuery = 'House';
  static const _houseGenreQueries = [
    'House',
  ];
  static const _buzzingLikeKey = 'buzzing_house_liked';
  ...
  Future<void> _fetchTrendingTracks() async {
    try {
      final resp = await dioClient.dio.get(
        '/discovery/trending',
        queryParameters: {'genre': _houseQuery},
      );
  ...
  Future<void> _fetchPlaylists() async {
    try {
      debugPrint('[HouseGenrePage] _fetchPlaylists: requesting genre="House"');
      final resp = await dioClient.dio.get(
        '/playlists',
        queryParameters: {'genre': 'House', 'limit': 10},
      );
      debugPrint('[HouseGenrePage] _fetchPlaylists raw: ${resp.data}');
      if (!mounted) return;
      setState(() {
        ...
        children: [
          ...
          if (previewTracks.isEmpty && recentTracks.isEmpty && _playlists.isEmpty)
            const Padding(
              ...
              child: Text(
                'No House content available right now.',
  ...
  void _openIntroducingPlaylist() {
    if (_playlists.isEmpty) {
      _pushIntroducingPlaylist('/search/house/introducing');
      return;
    }
    ...
    final selectedPlaylist = _playlists.firstWhere(
      (playlist) {
        ...
        return title.contains('buzzing house') ||
    ...
    _pushIntroducingPlaylist('/search/house/introducing?playlistId=$playlistId');
  }
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  gradient: RadialGradient(
                    ...
                    radius: 1.22,
                    colors: [
                      const Color(0xFFFFC2DD).withValues(alpha: 0.96),
                      const Color(0xFFFF8FC2).withValues(alpha: 0.88),
                      const Color(0xFFE85A9A).withValues(alpha: 0.68),
                      const Color(0xFF9E3D68).withValues(alpha: 0.46),
                      const Color(0xFF431E30).withValues(alpha: 0.24),
                    ],
  ...
  Widget _buildPlaylistListCard(_PlaylistInfo playlist) {
    ...
    return GestureDetector(
      onTap: () => context.push(
                '/search/house/introducing?playlistId=${Uri.encodeComponent(playlist.id)}',
              ),
      child: Container(
      ...
      children: [
        ...
        if (_isPlaylistsLoading)
          ...
          const Padding(
            ...
            child: Text(
              'No House playlists available right now.',
        ...
        if (_isAlbumsLoading)
          ...
          Padding(
            ...
            child: _buildInlineRetryCard(
              message: 'Failed to load House albums.',
          ...
          const Padding(
            padding: EdgeInsets.symmetric(horizontal: 16, vertical: 8),
            child: Text(
              'No House albums available right now.',
              style: TextStyle(color: Colors.white54, fontSize: 14),
            ),
          )
        else
      ...
      children: [
        ...
        if (_isLoading)
          ...
          Padding(
            padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
            child: _buildInlineRetryCard(
              message: 'Failed to load House artist profiles.',
          ...
          const Padding(
            padding: EdgeInsets.symmetric(horizontal: 16, vertical: 8),
            child: Text(
              'No House artist profiles available right now.',
              style: TextStyle(color: Colors.white54, fontSize: 14),
            ),
          )
        else
      ...
      children: [
        ...
        if (_isLoading)
          ...
          Padding(
            padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
            child: _buildInlineRetryCard(
              message: 'Failed to load more House tracks.',
              onRetry: _fetchGenreTracks,
            ),
          )
          ...
          const Padding(
            padding: EdgeInsets.symmetric(horizontal: 16, vertical: 8),
            child: Text(
              'No more House tracks available right now.',
              style: TextStyle(color: Colors.white54, fontSize: 14),
            ),
          )
        else
  ...
  Widget _buildInlineRetryCard({
    ...
    String message = 'Failed to load House playlists.',
  }) {
    return Container(
      width: double.infinity,
