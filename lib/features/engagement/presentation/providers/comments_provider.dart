...
class CommentsNotifier extends StateNotifier<CommentsState> {
  ...
  Future<bool> postComment({
    ...
    try {
      var newComment = await _dataSource.postComment(
        trackId: _trackId,
      ...
      // When the server returns user as a plain ID string (not a populated
      // object) CommentModel.fromJson falls back to displayName:'Unknown'.
      // Enrich immediately with the current session's display name so the
      // comment list never shows "Unknown" right after posting.
      if (newComment.user.displayName == 'Unknown' ||
          newComment.user.displayName.isEmpty) {
        final sessionName = await UserSession.getDisplayName();
        final sessionId = await UserSession.getUserId();
        if (sessionName != null && sessionName.isNotEmpty) {
          newComment = CommentModel(
            ...
            user: CommentUserModel(
              id: newComment.user.id.isNotEmpty
                  ? newComment.user.id
                  : (sessionId ?? ''),
